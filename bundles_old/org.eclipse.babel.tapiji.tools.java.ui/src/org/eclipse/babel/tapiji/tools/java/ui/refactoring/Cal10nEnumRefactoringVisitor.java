...
/**
 * Changes the enum class, which is referenced by the Cal10n framework. It
...
public class Cal10nEnumRefactoringVisitor extends ASTVisitor {
     ...
     */
    public Cal10nEnumRefactoringVisitor(CompilationUnit enumCu, String oldKey,
            String newKey, List<String> changeSet) {
        this.oldKey = oldKey;
        this.newKey = newKey;
        this.enumCu = enumCu;
    ...
    public boolean visit(EnumConstantDeclaration node) {
        ...
        if (node.resolveVariable().getName().equals(oldKey)) {
            ...
            EnumConstantDeclaration newDeclaration = ast
            ...
            try {
                ...
                if (textEdit.hasChildren()) { // if the compilation unit has
                                              // been
                                              // changed
                    ICompilationUnit icu = (ICompilationUnit) enumCu
                            .getJavaElement();
