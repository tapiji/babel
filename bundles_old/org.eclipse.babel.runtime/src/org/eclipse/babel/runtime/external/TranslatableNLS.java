...
public abstract class TranslatableNLS {
  ...
  private static ITranslatableText convert(Object binding) {
    return (binding instanceof ITranslatableText) ? (ITranslatableText) binding : new NonTranslatableText(String.valueOf(binding));
  }
  ...
  private static ITranslatableText internalBind(ITranslatableText message, Object... args) {
    if (message == null) return new NonTranslatableText("No message available."); //$NON-NLS-1$
    if (args == null || args.length == 0) args = EMPTY_ARGS;
  ...
  static void log(int severity, String message, Exception e) {
    ...
    if (e != null) statusMsg += ": " + e.getMessage(); //$NON-NLS-1$
  ...
  private static class MessagesProperties extends Properties {
    ...
    public synchronized Object put(String key, Object value) {
      ...
      if (fieldObject == ASSIGNED) return null;
      if (fieldObject == null) {
      ...
      if ((field.getModifiers() & MOD_MASK) != MOD_EXPECTED) return null;
      try {
