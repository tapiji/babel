...
public final class PreferenceGeneralPage extends APreferencePage {
	...
	autoAdjustTextView(composite);

	initValues();
	return composite;
    }

    private void autoAdjustTextView(Composite composite) {
	final Composite field = createFieldComposite(composite);
	autoAdjustTextView = new Button(field, SWT.CHECK);
	createLabel(field, "Auto adjust text view");
    }
	...
	final Composite field = createFieldComposite(composite);
	createLabel(field, "Minimale Editor-H�he");
	...
	autoAdjustTextView.setSelection(PropertyPreferences.getInstance().isAdjustTextView());
    }
