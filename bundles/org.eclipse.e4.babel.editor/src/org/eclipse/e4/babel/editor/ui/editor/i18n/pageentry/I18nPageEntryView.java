...
public final class I18nPageEntryView extends Composite implements KeyListener, TraverseListener, FocusListener, MouseListener, I18nPageEntryContract.View, ITextListener {
	...
	similarButton.addListener(SWT.Selection, (e) -> {
	    ...
	    showVisitorDialog("Ähnliche Werte gefunden. Klicken für Details.",
		    "Die Werte folgender Schl\u00FCssel sind innerhalb der Lokalen \"{1}\" beim Schl\u00FCssel \"{0}\" ähnlich:", foundKeys);
	});
    }

    private void showVisitorDialog(String head, String message, StringBuilder foundKeys) {
	final String body = MessageFormat.format(message, presenter.getActiveKey(), UIUtils.getDisplayName(presenter.getLocale())) + foundKeys.toString();
