		...
		for (int i = 0; i < lines.length; i++) {
			...
			if (equalPosition >= 1 && (isRegularLine || isCommentedLine)) {
				...
				while (lineBuf.lastIndexOf(backslash) == lineBuf.length() - 1) {
					...
					if (++i < lines.length) {
						String wrappedLine = PATTERN_LEADING_SPACE.matcher(lines[i]).replaceFirst("");
						//                        String wrappedLine = lines[i].trim();
						if (isCommentedLine) {
							lineBuf.append(PATTERN_COMMENT_START.matcher(wrappedLine).replaceFirst(""));
						} else {
				...
				bundle.addBundleEntry(BundleEntry.create(key, value, comment, isCommentedLine));
				// parse comment line
					...
					for (int i = 0; i < 4; i++) {
						...
						default:
							...
							Log.e(TAG, new Exception(
									"PropertiesDeserializer bad character encoding for string: ".concat(str)));
						}
					}
