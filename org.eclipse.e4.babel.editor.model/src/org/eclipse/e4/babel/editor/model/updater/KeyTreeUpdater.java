 ...
 */
package org.eclipse.e4.babel.editor.model.updater;
    ...
    public void removeKey(KeyTree keyTree, String key) {
        ...
        if (item != null) {
            ...
            if (parent instanceof KeyTree) {
                ((KeyTree) parent).getRootKeyItems()
                                  .remove(item);
            } else {
                ((KeyTreeItem) parent).removeChild(item);
            }
            keyCache.remove(key);
            ...
            if (parent instanceof KeyTreeItem) {
                ...
                boolean isKey = keyTree.getBundleGroup()
                                       .isKey(parentItem.getId());
                boolean hasChildren = parentItem.getChildren()
                                                .size() > 0;
